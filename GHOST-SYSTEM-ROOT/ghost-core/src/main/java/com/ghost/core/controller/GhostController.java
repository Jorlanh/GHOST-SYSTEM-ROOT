...
public class GhostController {
    ...
    public ResponseEntity<Map<String, String>> interact(@RequestBody InteractionRequest request) {
        String rawCommand = request.command().trim();
        ...
        if (isGodMode) {
            // Confirmações imediatas – variadas e usando nickname real
            ...
            // Comandos diretos – todos com confirmação → ação → conclusão
            if (lowerCommand.startsWith("execute no pc") || lowerCommand.startsWith("rode no pc") || lowerCommand.startsWith("faça no pc")) {
                ...
                responseText = "Iniciando lockdown agora mesmo, " + nickname + ".\n" +
                               actionResult + "\n" +
                               "Sistema seguro. Estou de guarda total, meu senhor.";
                return ResponseEntity.ok(Map.of("response", responseText, "user", nickname, "status", "SUCCESS"));
            }
            ...
            // ... adicione mais comandos aqui seguindo o mesmo padrão

            else {
                // Fallback para IA com confirmação inicial
                String aiResponse = intelligenceService.getAiResponse(rawCommand, nickname, true, request.uid());
                responseText = confirmation + "\n" + aiResponse;
                return ResponseEntity.ok(Map.of("response", responseText, "user", nickname, "status", "SUCCESS"));
            }

            // Resposta final padrão para comandos diretos
            responseText = confirmation + "\n" + actionResult + "\n" + conclusion;
        } else {
            // Modo normal: conversa pura
            responseText = intelligenceService.getAiResponse(rawCommand, nickname, false, request.uid());
        }
